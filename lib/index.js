...
// Load native binary from dist/ — the single source of truth for built artifacts.
// npm run build produces the binary here; no fallbacks to prevent confusion.
...
function decodeWithNative(buffer, formatHint, options) {
    if (!nativeBinding) {
        throw new Error('Native module not built. Run "npm run setup" and "npm run build".');
    }
    // Support new signature: decode(buffer, options) or decode(buffer, formatHint, options)
    if (typeof formatHint === 'object' && formatHint !== null) {
        // decode(buffer, options) - formatHint is actually options
        return nativeBinding.decode(buffer, formatHint);
    }
    if (options && typeof options === 'object') {
        // decode(buffer, formatHint, options)
        return nativeBinding.decode(buffer, formatHint, options);
    }
    // Legacy: decode(buffer, formatHint)
    return nativeBinding.decode(buffer, formatHint);
}
...
class NImagePipeline {
        ...
        if (nativeBinding && !standardFormats.has(format)) {
            ...
            if (needsNativeDecode || format === 'unknown') {
                try {
                    let imageData;
                    try {
                        imageData = nativeBinding.decode(buffer);
                    } catch (nativeErr) {
                        if (IMAGEMAGICK_FORMATS.has(format) || format === 'unknown') {
                            // CLI fallback for edge cases
                            const tempIn = path.join(os.tmpdir(), `nimage_in_${Date.now()}_${Math.floor(Math.random()*10000)}`);
                            const tempOut = path.join(os.tmpdir(), `nimage_out_${Date.now()}_${Math.floor(Math.random()*10000)}.png`);
                            try {
                                fs.writeFileSync(tempIn, buffer);
                                let magickPath = path.join(__dirname, '..', 'dist', 'magick.exe');
                                if (!fs.existsSync(magickPath)) {
                                    magickPath = path.join(__dirname, '..', 'deps', 'bin', 'magick.exe');
                                }
                                const result = spawnSync(magickPath, [tempIn, tempOut]);
                                if (result.status !== 0) {
                                    throw new Error(`ImageMagick decode failed: ${result.stderr ? result.stderr.toString() : 'Unknown error'}`);
                                }
                                const outBuffer = fs.readFileSync(tempOut);
                                this._sharp = sharp(outBuffer);
                                return;
                            } finally {
    ...
    blur(sigma) {
        return this._addOperation('blur', [sigma]);
    }
     ...
     * RAW/HEIC: native decode + Sharp resize
     * Standard formats (JPEG/PNG/WebP): Sharp resize
     */
    async thumbnail(options = {}) {
        ...
        const rawFormats = new Set(['cr2', 'nef', 'arw', 'orf', 'raf', 'rw2', 'dng', 'pef', 'srw', 'rwl', 'crw', 'mrw', 'nrw', 'erf', 'heic', 'heif']);
        ...
        if (rawFormats.has(format) && nativeBinding) {
            let thumb = null;
            let thumbError = null;
            
            for (const trySize of [size, 4096]) {
                try {
                    thumb = nativeBinding.thumbnail(buffer, { size: trySize });
                    if (thumb && thumb.width > 0 && thumb.height > 0 && thumb.data) {
                        break;
                    }
                } catch (e) {
                    thumbError = e;
            ...
            if (thumb && thumb.width > 0 && thumb.height > 0 && thumb.data) {
                ...
                return sharp(Buffer.from(thumb.data), {
                    raw: {
                        width: thumb.width,
                        height: thumb.height,
                        channels: 3
                    ...
                    .toBuffer()
                    .then(data => ({
                        width: resizeOptions.width || Math.round(thumb.width * (size / thumb.height)),
                        height: resizeOptions.height || Math.round(thumb.height * (size / thumb.width)),
                        channels: 3,
                        bitsPerChannel: 8,
                        hasAlpha: false,
                        colorSpace: 'sRGB',
            ...
            return sharp(Buffer.from(imageData.data), {
                ...
                .toBuffer()
                .then(data => ({
                    width: resizeOptions.width || Math.round(imageData.width * (size / imageData.height)),
                    height: resizeOptions.height || Math.round(imageData.height * (size / imageData.width)),
                    channels: 3,
                    bitsPerChannel: 8,
                    hasAlpha: false,
                    colorSpace: 'sRGB',
        ...
        throw new Error(`Unsupported format for thumbnail: ${format}`);
    }
...
async function extractThumbnail(input, options = {}) {
    ...
    if (!sharp) {
        throw new Error('Sharp is required for thumbnail extraction');
    }
...
// ============================================================================
// Capabilities (static JSON, no native dependency)
// ============================================================================
...
function getCapabilities() {
    ...
    try {
        capabilitiesCache = require(path.join(__dirname, 'capabilities', 'index.json'));
    } catch (e) {
